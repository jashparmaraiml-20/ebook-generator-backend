...
def run_pipeline(
    ...
    print(f"  {Fore.GREEN}╚══════════════════════════════════════════╝{Style.RESET_ALL}")
    total_words = sum(len(ch.get('content', '').split()) for ch in content_result.get('chapters', []))
    images_count = sum(1 for ch in images_result.get('chapters', []) if ch.get('image_path'))
    
    print(f"\n  {Fore.YELLOW}Title:{Style.RESET_ALL}      {brief.get('title', 'Unknown')}")
    print(f"  {Fore.YELLOW}Category:{Style.RESET_ALL}   {brief.get('category', 'Unknown')}")
    print(f"  {Fore.YELLOW}Chapters:{Style.RESET_ALL}   {len(content_result.get('chapters', []))}")
    print(f"  {Fore.YELLOW}Words:{Style.RESET_ALL}      ~{total_words:,}")
    print(f"  {Fore.YELLOW}Images:{Style.RESET_ALL}     {images_count}")
